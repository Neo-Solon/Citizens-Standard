"""
paper03_transition — verifies that `transition_replication/` still reproduces the numbers the paper publishes.

The package itself is NOT copied here. It stays where the repo keeps it, at
`replication/transition_replication/`, and remains independently runnable. This adapter only drives it:
...
PKG       = REPL / "transition_replication"
ENTRY     = 'run_all.py'
ARTIFACTS = [('json', 'cs_debt_band/dsa_locked.json')]
