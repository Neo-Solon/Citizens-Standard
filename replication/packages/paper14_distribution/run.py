"""
paper14_distribution — verifies that `distribution_inequality_replication/` still reproduces the numbers the paper publishes.

The package itself is NOT copied here. It stays where the repo keeps it, at
`replication/distribution_inequality_replication/`, and remains independently runnable. This adapter only drives it:
...
PKG       = REPL / "distribution_inequality_replication"
ENTRY     = 'run_all.py'
ARTIFACTS = [('json', 'results/inequality_results.json')]
